...
function displayMortgageTable(termLength, monthlyPayment) {
    ...
    for (let i = 1; i <= termLength; i++) {
        ...
        tableCells[4].textContent = `$${runningInterestArray}`;
        tableCells[5].textContent = `$${balanceArray.toLocaleString()}`;
...
function displayOverview(monthlyPayment) {
        ...
        let totalCost = totalInterest + totalPrinciple;
        ...
        totalCostSection.innerHTML = `Total Cost: $${totalCost.toLocaleString()}`
